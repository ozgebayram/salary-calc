...
function Deneme() {
  ...
  const calculateFinalSalary = () => {
    ...
    if (department === "ogretmen") {
	  ...
	  if (yearsOfWork >= 5) {
		yearsOfWorkFactor += addedSalary * 0.05;
	  }
	
	  if (yearsOfWork >= 10) {
		yearsOfWorkFactor += addedSalary * 0.1;
	  }
    ...
    <Container className="mt-5">
      <Form>
        ...
        <Form.Group controlId="formYearsOfWork" className="mb-3">
          <Form.Label>Çalışma Yılı</Form.Label>
          <Form.Control
            type="number"
            value={yearsOfWork}
        ...
        <Button variant="primary" onClick={calculateFinalSalary} className="mt-3">
          Hesapla
        </Button>
      </Form>
      ...
      {calculatedData && (
        <div className="mt-3">
          <h4>Toplam Maaş: ₺ {calculatedData.finalSalary.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</h4>
          ...
          <Table striped bordered hover className="mt-3">
            ...
            <tbody>
              <tr>
                ...
                <td>{calculatedData.childCountSalary.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</td>
                <td>{calculatedData.educationLevel}</td>
                <td>{calculatedData.department}</td>
                <td>{finalSalary.toFixed(0).replace(/\B(?=(\d{3})+(?!\d))/g, ".")}</td>
              </tr>
            </tbody>
          </Table>
        </div>
